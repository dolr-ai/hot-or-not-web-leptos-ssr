...
pub fn ModelDropdownV2(
    ...
    let provider_icon = Signal::derive(move || {
        selected_provider
            .get()
            .as_ref()
            .and_then(|p| p.model_icon.clone())
    });
                ...
                <div class="absolute top-full left-0 right-0 mt-1 bg-[#212121] border border-neutral-800 rounded-lg shadow-lg z-50 py-1 max-h-[264px] overflow-y-auto scrollbar-thin scrollbar-thumb-neutral-600 scrollbar-track-transparent">
                    <For
                        ...
                        children=move |provider| {
                            ...
                            let duration_display = if provider.allowed_durations.is_empty() {
                                "Variable".to_string()
                            } else if provider.allowed_durations.len() == 1 {
                                format!("{}s", provider.allowed_durations[0])
                            } else {
                                format!("{}-{}s",
                                    provider.allowed_durations.iter().min().unwrap_or(&0),
                                    provider.allowed_durations.iter().max().unwrap_or(&0)
                                )
                            };
                                                    ...
                                                    if is_available {
                                                        view! {
                                                            ...
                                                            <div class="flex items-center gap-1 px-1 py-1 rounded border border-neutral-700">
                                                                ...
                                                                <span class="text-neutral-400 text-xs">{duration_display}</span>
                                                            </div>
                                                        }.into_any()
                                                    } else {
                                                        view! {
