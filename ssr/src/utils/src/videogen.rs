...
use videogen_common::{VideoGenClient, VideoGenRequest, VideoGenResponse};
...
pub fn sign_videogen_request(
    ...
    let msg = videogen_request_msg(request);
...
pub async fn generate_video_with_balance_check(
    ...
    let client =
        VideoGenClient::with_bearer_token(OFF_CHAIN_AGENT_URL.as_str().to_string(), bearer_token);

    let video_response = client.generate(signed_request.request).await.map_err(|e| {
        ...
        tokio::spawn(async move {
            if let Err(rollback_err) =
                rollback_videogen_balance(rollback_principal, rollback_balance, rollback_cost).await
            {
                eprintln!(
                    "Failed to rollback balance for user {}: {}",
                    rollback_principal, rollback_err
                );
            }
        });
