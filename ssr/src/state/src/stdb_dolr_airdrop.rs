...
use yral_spacetime_bindings::{
    autogenerated::backend::{self, mark_airdrop_claimed, mark_sats_airdrop_claimed},
...
impl WrappedContext {
    pub async fn new(token: Option<impl Into<String>>) -> anyhow::Result<Self> {
        ...
        let tx_sats_airdrop = tx.clone();
        let tx_dolr_airdrop = tx.clone();

        conn.reducers.on_mark_sats_airdrop_claimed(
            move |event, user_principal, now, last_airdrop_at| {
                let search_hash = fast_hash(SatsAirdropInput {
                ...
                println!("on mark sats callback with principal: {user_principal} and res {res:?}");

                // channel must be not be closed
                tx_sats_airdrop.send((search_hash, res)).unwrap();
            },
        );

        conn.reducers
            .on_mark_airdrop_claimed(move |event, user_principal, duration, now| {
                let search_hash = fast_hash(DolrAirdropInput {
                    user_principal: user_principal.clone(),
                    duration: *duration,
                    now: *now,
                });
                ...
                // channel must be not be closed
                tx_dolr_airdrop.send((search_hash, res)).unwrap();
            });
    ...
    pub async fn mark_sats_airdrop_claimed(
        &self,
        user_principal: Principal,
        now: SystemTime,
        ...
        let search_hash = fast_hash(SatsAirdropInput {
            user_principal: user_principal.to_text(),
            last_airdrop_at: last_airdrop_at.map(Into::into),
            now: now.into(),
        });
        let mut rx = self.tx.subscribe();
        self.conn
        ...
        println!("reducer took {:?} to mark sats airdrop", now.elapsed());
